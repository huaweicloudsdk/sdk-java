...
/**
 * Most are copied from {@link DefaultEndpointURLResolver}, 
 * Add features to explicit override endpoint of services
 *
 * @author QianBiao.NG
 * @date   2017-06-10 18:14:04
 */
public class OverridableEndpointURLResolver implements EndpointURLResolver {

	static final Logger LOG = LoggerFactory.getLogger(OverridableEndpointURLResolver.class);
	static final Map<Key, String> CACHE = new ConcurrentHashMap<Key, String>();
	static boolean LEGACY_EP_HANDLING = Boolean.getBoolean(LEGACY_EP_RESOLVING_PROP);
	String publicHostIP;

	/** service override endpoint mapping */
	Map<ServiceType, String> overrides = Maps.newHashMap();

	/**
	 * register the override endpoint URL for service-type
	 * 
	 * @param serviceType
	 * @param endpoint
	 */
	public void addOverrideEndpoint(ServiceType serviceType, String endpoint) {
		overrides.put(serviceType, endpoint);
	}

	public Map<ServiceType, String> getOverrides() {
		return overrides;
	}
